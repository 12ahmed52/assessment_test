...
private:
  ...
  void publish_odometry(const ros::Time& stamp, const Eigen::Matrix4f& pose) {
    static std::vector<Eigen::Vector3f> translation_window;
    static std::vector<Eigen::Quaternionf> rotation_window;
    const size_t window_size = 5; // Example window size

    Eigen::Vector3f current_translation = pose.block<3, 1>(0, 3);
    Eigen::Quaternionf current_rotation(pose.block<3, 3>(0, 0));

    // Add current translation and rotation to their respective windows
    if (translation_window.size() >= window_size) translation_window.erase(translation_window.begin());
    translation_window.push_back(current_translation);

    if (rotation_window.size() >= window_size) rotation_window.erase(rotation_window.begin());
    rotation_window.push_back(current_rotation);

    // Filter translation by median
    std::vector<Eigen::Vector3f> sorted_translations = translation_window;
    std::nth_element(sorted_translations.begin(), sorted_translations.begin() + sorted_translations.size() / 2, sorted_translations.end(), [](const Eigen::Vector3f& a, const Eigen::Vector3f& b) {
        return a.norm() < b.norm();
    });
    Eigen::Vector3f filtered_translation = sorted_translations[sorted_translations.size() / 2];

    // Filter rotation by averaging (simplified method)
    Eigen::Quaternionf filtered_rotation = rotation_window.front();
    for (size_t i = 1; i < rotation_window.size(); ++i) {
        if (filtered_rotation.dot(rotation_window[i]) < 0) { // Ensure shortest path
            rotation_window[i].coeffs() = -rotation_window[i].coeffs();
        }
        filtered_rotation = filtered_rotation.slerp(1.0f / (i + 1), rotation_window[i]);
    }

    // Construct filtered pose matrix
    Eigen::Matrix3f rotation_matrix = filtered_rotation.toRotationMatrix();
    Eigen::Matrix4f filtered_pose = Eigen::Matrix4f::Identity();
    filtered_pose.block<3, 3>(0, 0) = rotation_matrix;
    filtered_pose.block<3, 1>(0, 3) = filtered_translation;

    // Now, use filtered_pose instead of pose for the transformations and publishing
    // Start of your existing code adapted for filtered_pose

    // broadcast the transform over tf
    if(tf_buffer.canTransform(robot_odom_frame_id, odom_child_frame_id, ros::Time(0))) {
        geometry_msgs::TransformStamped map_wrt_frame = tf2::eigenToTransform(Eigen::Isometry3d(filtered_pose.inverse().cast<double>()));
        ...
        geometry_msgs::TransformStamped odom_trans = tf2::eigenToTransform(Eigen::Isometry3d(filtered_pose.cast<double>()));
        odom_trans.header.stamp = stamp;
        odom_trans.header.frame_id = "map";
    ...
    tf::poseEigenToMsg(Eigen::Isometry3d(filtered_pose.cast<double>()), odom.pose.pose);
